...
interface RestaurantCardProps {
  restaurant: {
    ...
    cuisine: string;
    priceRange: string;
    price_range?: string; // backend may use this
    rating?: number;
    reviewCount?: number;
    location: string;
    image: string;
...
export default function RestaurantCard({ restaurant }: RestaurantCardProps) {
  ...
  // Ensure we have an array for slots, fallback to empty
  const slots = restaurant.availableSlots || [];

  // Use priceRange or price_range
  const price = restaurant.priceRange || restaurant.price_range || "$$";

  // Use actual rating or default 0
