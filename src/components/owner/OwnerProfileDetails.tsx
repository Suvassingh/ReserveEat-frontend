...
export default function OwnerProfileDetails({
  ...
  useEffect(() => {
    if (restaurant) {
      ...
      setPriceRange(restaurant.price_range || 1000);
      setLocation(restaurant.location || "");
      setAddress(restaurant.address || "");
      setChef(restaurant.chef || "");
      setTags(restaurant.tags || "");
      setTotalSeats(restaurant.total_seats?.toString() || "20");
      setAvailableSlots(restaurant.available_slots || []);
  ...
  const handleUpdateRestaurant = async (e: React.FormEvent) => {
    ...
    try {
      ...
      formData.append("price_range", String(priceRange));
      formData.append("location", location);
      formData.append("address", address);
      formData.append("chef", chef);
      formData.append("tags", tags);
      formData.append("available_slots", JSON.stringify(availableSlots));
